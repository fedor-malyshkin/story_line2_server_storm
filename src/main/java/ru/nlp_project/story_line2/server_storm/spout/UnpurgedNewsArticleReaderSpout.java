 ...
 * Схема работы таковы:
 * <ol>
 * <li>Выбираются записи (!images_purged && !in_process && "дата публикации меньше указанной")</li>
 * <li>выбранная записи запускаются в поток с установкой флага "in_process"</li>
 * <li>после отработки при положительном исходе выставляется in_process = false </li>
 * <li>после отработки при отрицательном исходе выставляется in_process = false</li>
 * <li>после того как не удалось получить ни одной записи делается пауза на сутки</li>
 * </ol>
 * 
 * 
 * @author fedor
 *
 */
public class UnpurgedNewsArticleReaderSpout implements IRichSpout {
	...
	public void nextTuple() {
		...
		try {
			...
			Map<String, Object> newsArticle =
					mongoDBClient.getNextUnpurgedImagesNewsArticle(DateTimeUtils.asDate(localDate));
			// there is no data
			if (null == newsArticle) {
				return;
			}
			collector.emit(Arrays.asList(NewsArticle.idString(newsArticle)),
	...
	public void ack(Object msgId) {
		try {
			mongoDBClient.unmarkNewsArticleAsInProcess((String) msgId);
			mongoDBClient.markNewsArticleAsImagesPurged((String) msgId);
		} catch (Exception e) {
